		...
		return {
			...
			create : function(url,ctrlr,data,opts){
				...
				return $modal.open({
					...
					keyboard : k,
					backdrop: b,
					windowClass: angular.isDefined(opts.windowClass) ? opts.windowClass : '',
					resolve : {
...
angular.module('dialogs',['dialogs.services','ngSanitize']) // requires angular-sanitize.min.js (ngSanitize) //code.angularjs.org/1.2.1/angular-sanitize.min.js
	...
	.run(['$templateCache',function($templateCache){
		...
		$templateCache.put('/dialogs/notify.html',  '<div id="notifyModal"  role="dialog" aria-Labelledby="notifyModalLabel"><div class="modal-content"><div class="modal-header dialog-header-notify"><button type="button" class="close" ng-click="close()" class="pull-right">&times;</button><h4 class="modal-title text-info"><span class="glyphicon glyphicon-info-sign"></span> {{header}}</h4></div><div class="modal-body text-info" ng-bind-html="msg"></div><div class="modal-footer"><button type="button" class="btn btn-primary" ng-click="close()">OK</button></div></div></div>');
		...
		$templateCache.put('/dialogs/confirm.html', '<div id="confirmModal" role="dialog" aria-Labelledby="confirmModalLabel"><div class="modal-content"><div class="modal-header dialog-header-confirm" style="background-color:#f6f6f6"><div class="btn pull-right" data-ng-click="no()" style="margin-right:-15px;"><i class="fontello-icon-cancel-circle"></i></div><h4 class="modal-title" style="color:#000">{{header}}</h4></div><div class="modal-body" ng-bind-html="msg"></div><div class="modal-footer"><button type="button" class="btn btn-primary" ng-click="yes()">Yes</button><button type="button" class="btn btn-default" ng-click="no()">No</button></div></div></div>');
	}]); // end run / dialogs
