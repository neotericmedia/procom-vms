(function (angular) {
    ...
    angular.module('phoenix.vms.controllers').controller('VmsDiscountPreprocessedController', VmsDiscountPreprocessedController);

    VmsDiscountPreprocessedController.$inject = ['$rootScope', '$scope', '$sce', '$state', 'common', 'config', '$filter', '$timeout', 'NavigationService', 'dialogs', 'TransactionApiService', 'CodeValueService', '$stateParams', '$q', 'VmsApiService', 'WorkflowApiService', 'commonDataService'];

    function VmsDiscountPreprocessedController($rootScope, $scope, $sce, $state, common, config, $filter, $timeout, NavigationService, dialogs, TransactionApiService, CodeValueService, $stateParams, $q, VmsApiService, WorkflowApiService, commonDataService) {

        $scope.$on('vmsPreprocessedDocumentChange', function (event, data) {
            if (data.entityTypeId == ApplicationConstants.EntityType.VmsDiscountImportedRecord) {
            ...
            if ($scope.documentPublicId) {
                var oDataParams = oreq.request()
                    .withSelect([
                        ...
                        'Note',
                        'VmsDiscountImportedRecordTypeIdInitial',
                        'VmsDiscountImportedRecordTypeIdFinal',
                        'Reason',
                        'UploadedDatetime',
                        'LastModifiedDatetime'
                    ]).url();
                return TransactionApiService.getVmsDiscountImportedRecordsTable($scope.documentPublicId, oDataParams, tableState);
            }
            else {
                return $q.reject();
            }
        ...
        $scope.markAsProcess = function (entity) {
            $scope.processing = true;
            TransactionApiService.vmsDiscountImportRecordTypeUpdate({ LastModifiedDatetime: entity.LastModifiedDatetime, Id: entity.Id, VmsDiscountImportedRecordTypeIdFinal: ApplicationConstants.VmsDiscountImportedRecordType.ToProcess }).then(function (response) {
                var record = $scope.items[$scope.items.map(function (x) { return x.Id; }).indexOf(entity.Id)];
                if (record) {
                    record.VmsDiscountImportedRecordTypeIdFinal = ApplicationConstants.VmsDiscountImportedRecordType.ToProcess;
                ...
                TransactionApiService.vmsDiscountImportRecordTypeUpdate(
                    {
                        ...
                        VmsDiscountImportedRecordTypeIdFinal: ApplicationConstants.VmsDiscountImportedRecordType.Conflict,
                        UserNotes: reason
                    }
                )
                    .then(function (response) {
                        var record = $scope.items[$scope.items.map(function (x) { return x.Id; }).indexOf(entity.Id)];
                        if (record) {
                            record.VmsDiscountImportedRecordTypeIdFinal = ApplicationConstants.VmsDiscountImportedRecordType.Conflict;
                ...
                TransactionApiService.vmsDiscountImportRecordTypeUpdate(
                    {
                        Id: entity.Id,
                        LastModifiedDatetime: entity.LastModifiedDatetime,
                        VmsDiscountImportedRecordTypeIdFinal: ApplicationConstants.VmsDiscountImportedRecordType.Discarded,
                        UserNotes: reason
                    }
                )
                    .then(function (response) {
                        var record = $scope.items[$scope.items.map(function (x) { return x.Id; }).indexOf(entity.Id)];
                        if (record) {
                            record.VmsDiscountImportedRecordTypeIdFinal = ApplicationConstants.VmsDiscountImportedRecordType.Discarded;
                            record.Reason = reason;
                            updateRecords(record);
                        }
                    }, function (error) { }).finally(function () {
        ...
        var updateRecords = function (record) {
            ...
            if ($scope.tableState.search && $scope.tableState.search.predicateObject) {
                if (!remove && $scope.tableState.search.predicateObject.VmsDiscountImportedRecordTypeIdFinal) {
                    if ($scope.tableState.search.predicateObject.VmsDiscountImportedRecordTypeIdFinal.indexOf(record.VmsDiscountImportedRecordTypeIdFinal) == -1) {
                        remove = true;
                    }
                }
            ...
            if (remove) {
                ...
                TransactionApiService.getVmsDiscountImportedRecord(record.Id).then(
                    function (response) {
