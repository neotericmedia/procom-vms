(function (angular, app) {
    ...
    angular.module('phoenix.vms.controllers').controller('VmsDiscountConflictsController', VmsDiscountConflictsController);

    angular.module('phoenix.vms.controllers').filter('vmsRate', VmsRateFilter);

    VmsDiscountConflictsController.$inject = ['AssignmentApiService', 'clientOrganizations', 'CodeValueService', 'common', 'dialogs', 'vmsNewTableState', 'JournalApiService', 'mixinsFactory', 'NavigationService', 'TransactionApiService', 'vmsTableParams', '$scope', '$stateParams', 'VmsApiService', 'WorkflowApiService', 'phoenixsocket'];

    function VmsDiscountConflictsController(AssignmentApiService, clientOrganizations, CodeValueService, common, dialogs, vmsNewTableState, JournalApiService, mixinsFactory, NavigationService, TransactionApiService, vmsTableParams, $scope, $stateParams, VmsApiService, WorkflowApiService, phoenixsocket) {

        NavigationService.setTitle('Discount Transactions with conflicts', 'icon icon-transaction');
        ...
        angular.extend(self, {
            ...
            actions: [{ Id: 1, CommandName: 'BatchThreadExecutionOnVmsDiscountRecordToChangeInternalOrg', DisplayName: 'Inter-Company Change' }, { Id: 2, CommandName: 'BatchThreadExecutionOnVmsDiscountRecordToDiscardedType', DisplayName: 'Discard' }],
            actionId: null,
        ...
        mixinsFactory.createTableSupportMixin(angular.extend({
            serviceMethod: TransactionApiService.getVmsDiscountConflicts
        ...
        self.successfulRetrieval = function (items, tableState) {
            ...
            if (self.isLoaded && tableState && tableState.search && tableState.search.predicateObject && Object.keys(tableState.search.predicateObject).length > 0 && !angular.equals(self.searchObject, tableState.search.predicateObject)) {
                ...
                angular.element('#vms-discount-table-body tr').removeClass('vms-selected');
            }
        ...
        self.clientChanged = function (organizationClient) {
            ...
            var isItemSelected = angular.element('#vmsDiscountClientOrg' + organizationClient.OrganizationIdClient).hasClass('vms-client-selected');
        ...
        self.restoreActions = function () {
            ...
            self.vmsBatchArray = [];
            self.OrganizationIdInternalChange = null;
            angular.element('#vms-discount-table-body tr').removeClass('vms-action-selected');
        };
        ...
        function filterTransactions(vmsItem) {
            ...
            self.billingTransactions = _.chain(self.selectedParentRow.VmsBillingTransactions).filter(function (bt) {
                bt.StartDate = bt.StartDate ? moment(bt.StartDate).toDate() : null;
                bt.EndDate = bt.EndDate ? moment(bt.EndDate).toDate() : null;
                return bt.BillingTransactionId > 0 && (bt.StartDate && bt.EndDate) &&
        ...
        function rowChecked(bt, index) {
            ...
            angular.forEach(self.billingTransactions, function (bt, idx) {
                bt.IsSelected = false;
            });
            if (isSelected) {
                bt.IsSelected = true;
        ...
        function actionButtonsHandler(commandName) {
            switch (commandName) {
                case "Change":
                    executeBatch(
                        { CommandName: 'BatchPreExecutionOnVmsDiscountRecordToChangeInternalOrg', WorkflowPendingTaskId: -1, OrganizationIdInternal: self.OrganizationIdInternalChange },
                        { CommandName: 'BatchThreadExecutionOnVmsDiscountRecordToChangeInternalOrg', OrganizationIdInternal: self.OrganizationIdInternalChange }
                        );
                    break;
                case "Discard":
                    executeBatch(
        ...
        function executeBatch(commandBatchPreExecutionJsonBody, commandBatchThreadExecutionJsonBody) {
            ...
            WorkflowApiService.workflowBatchOperationOnTasksSelected({
                ...
                NotifyName_BatchOperation_OnBatchMarkered: "VmsDiscountConflictsBatchMark",
        ...
        function resolveConflict() {
            ...
            if (self.conflictingRow) {
                ...
                TransactionApiService.vmsDiscountProcessedRecordSetTypeToProcess({ WorkflowPendingTaskId: self.selectedParentRow.WorkflowPendingTaskId, VmsDiscountRecordId: self.selectedParentRow.Id, BillingTransactionId: rowToResolve.BillingTransactionId }).then(
        ...
        function getExcelAll() {
            ...
            VmsApiService.csvStreamForAllConflictingVmsDiscountRecords(orgIdInternal, orgIdClient).then(function (response) {
                var fileName = self.OrganizationClientName + "_" +  dateString + "_DiscountConflicts.csv";
        ...
        function retrieveClients() {
            self.viewLoading = true;
            var vmsDataParams = oreq.request().withSelect(['OrganizationIdInternal', 'OrganizationIdClient', 'ClientOrgDisplayName', 'DiscountConflictCount']).url();
            TransactionApiService.getVmsAllItems(vmsDataParams).then(
                function (response) {
                    var items = _.filter(response.Items, function (item) {
                        return item.DiscountConflictCount > 0 && item.OrganizationIdInternal == $stateParams.internalOrganizationId;
                    });
        ...
        phoenixsocket.onPrivate("VmsDiscountConflictsBatchMark", function (event, data) {
    ...
    app.resolve.VmsDiscountConflictsController = {
