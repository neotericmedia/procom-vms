(function (angular) {
    ...
    angular.module('phoenix.vms.controllers').controller('VmsExpensePreprocessedController', VmsExpensePreprocessedController);

    VmsExpensePreprocessedController.$inject = ['$rootScope', '$scope', '$sce', '$state', 'common', 'config', '$filter', '$timeout', 'NavigationService', 'dialogs', 'TransactionApiService', 'CodeValueService', '$stateParams', '$q', 'VmsApiService', 'WorkflowApiService', 'commonDataService'];

    function VmsExpensePreprocessedController($rootScope, $scope, $sce, $state, common, config, $filter, $timeout, NavigationService, dialogs, TransactionApiService, CodeValueService, $stateParams, $q, VmsApiService, WorkflowApiService, commonDataService) {

        $scope.$on('vmsPreprocessedDocumentChange', function (event, data) {
            if (data.entityTypeId == ApplicationConstants.EntityType.VmsExpenseImportedRecord) {
        ...
        function buildPromise(tableState, isTruncated) {
            if ($scope.documentPublicId) {
                ...
                return VmsApiService.getVmsExpenseImportedRecordsTable($scope.documentPublicId, oDataParams, tableState);
            }
            else {
                return $q.reject();
            }
        ...
        $scope.markAsProcess = function (entity) {
            $scope.processing = true;
            VmsApiService.vmsExpenseImportRecordTypeUpdate({ LastModifiedDatetime: entity.LastModifiedDatetime, Id: entity.Id, VmsExpenseImportedRecordTypeIdFinal: ApplicationConstants.VmsExpenseImportedRecordType.ToProcess }).then(function (response) {
                var record = $scope.items[$scope.items.map(function (x) { return x.Id; }).indexOf(entity.Id)];
                if (record) {
                    record.VmsExpenseImportedRecordTypeIdFinal = ApplicationConstants.VmsExpenseImportedRecordType.ToProcess;
        ...
        $scope.markAsConflict = function (entity) {
            ...
            dlg.result.then(function (reason) {
                $scope.processing = true;
                VmsApiService.vmsExpenseImportRecordTypeUpdate(
                    {
                        Id: entity.Id,
                        LastModifiedDatetime: entity.LastModifiedDatetime,
                        VmsExpenseImportedRecordTypeIdFinal: ApplicationConstants.VmsExpenseImportedRecordType.Conflict,
                        UserNotes: reason
                    }
                )
                    .then(function (response) {
                        var record = $scope.items[$scope.items.map(function (x) { return x.Id; }).indexOf(entity.Id)];
                        if (record) {
                            record.VmsExpenseImportedRecordTypeIdFinal = ApplicationConstants.VmsExpenseImportedRecordType.Conflict;
                ...
                VmsApiService.vmsExpenseImportRecordTypeUpdate(
                    {
                        Id: entity.Id,
                        LastModifiedDatetime: entity.LastModifiedDatetime,
                        VmsExpenseImportedRecordTypeIdFinal: ApplicationConstants.VmsExpenseImportedRecordType.Discarded,
                        UserNotes: reason
                    }
                )
                    .then(function (response) {
                        var record = $scope.items[$scope.items.map(function (x) { return x.Id; }).indexOf(entity.Id)];
                        if (record) {
                            record.VmsExpenseImportedRecordTypeIdFinal = ApplicationConstants.VmsExpenseImportedRecordType.Discarded;
                            record.Reason = reason;
                            updateRecords(record);
                        }
                    }, function (error) { }).finally(function () {
        ...
        var updateRecords = function (record) {
            ...
            if ($scope.tableState.search && $scope.tableState.search.predicateObject) {
                if (!remove && $scope.tableState.search.predicateObject.VmsExpenseImportedRecordTypeIdFinal) {
                    if ($scope.tableState.search.predicateObject.VmsExpenseImportedRecordTypeIdFinal.indexOf(record.VmsExpenseImportedRecordTypeIdFinal) == -1) {
                        remove = true;
                    }
                }
            ...
            if (remove) {
                ...
                VmsApiService.getVmsExpenseImportedRecord(record.Id).then(
                    function (response) {
