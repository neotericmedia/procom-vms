(function (angular, app) {
    ...
    angular.module('phoenix.vms.controllers').controller('VmsExpenseProcessController', VmsExpenseProcessController);

    VmsExpenseProcessController.$inject = ['clientOrganizations', 'CodeValueService', 'common', 'dialogs', 'mixinsFactory', 'NavigationService', 'VmsApiService', '$stateParams', 'vmsTableParams', 'vmsNewTableState', 'WorkflowApiService', 'phoenixsocket', '$scope'];

    function VmsExpenseProcessController(clientOrganizations, CodeValueService, common, dialogs, mixinsFactory, NavigationService, VmsApiService, $stateParams, vmsTableParams, vmsNewTableState, WorkflowApiService, phoenixsocket, $scope) {

        NavigationService.setTitle('VMS Expense Processing', 'icon icon-transaction');
        ...
        mixinsFactory.createTableSupportMixin(angular.extend({
            serviceMethod: VmsApiService.getExpenseProcessedRecords
        ...
        self.workflowBatchOperationOnTasks = {
            notifyName: {
                NotifyName_BatchOperation_OnBatchMarkered: 'VmsExpenseTransactionCreateBatchMark'
            },
        };
        ...
        function createTransaction() {
            ...
            var taskIds = _.chain(self.vmsBatchArray)
                            ...
                            .filter(['TaskResultId', ApplicationConstants.TaskResult.VmsExpenseRecordGenerateTransaction])
            ...
            WorkflowApiService.workflowBatchOperationOnTasksSelected({
                ...
                CommandBatchPreExecutionJsonBody: {
                    CommandName: 'BatchPreExecutionOnVmsExpenseProcessedRecordToTransaction',
                    WorkflowPendingTaskId: -1
                },
                CommandBatchThreadExecutionJsonBody: {
                    CommandName: 'BatchThreadExecutionOnVmsExpenseRecordToTransaction'
                },
            }).then(
                function (success) {
        ...
        function sendToConflict(item, event) {
            ...
            dlg.result.then(function (reason) {
                self.viewLoading = true;
                VmsApiService.vmsExpenseRecordSetConflictType({ WorkflowPendingTaskId: item.WorkflowPendingTaskId, VmsExpenseRecordId: item.Id, ConflictReason: reason }).then(
                function (success) {
                    self.vmsBatchArray = [];
    ...
    app.resolve.VmsExpenseProcessController = {
        clientOrganizations: ['$q', 'VmsApiService', '$stateParams', function ($q, VmsApiService, $stateParams) {
            var result = $q.defer();
            var vmsDataParams = oreq.request()
                .withSelect(['OrganizationIdInternal', 'OrganizationIdClient', 'ClientOrgDisplayName', 'ExpensePreparedCount'])
                .url();
            VmsApiService.getVmsAllItems(vmsDataParams).then(
                function (response) {
                    var items = _.filter(response.Items, function (item) {
                        return item.ExpensePreparedCount > 0 && item.OrganizationIdInternal == $stateParams.internalOrganizationId;
                    });
