(function (angular) {
    ...
    angular.module('phoenix.vms.controllers').controller('VmsCommissionPreprocessedController', VmsCommissionPreprocessedController);

    VmsCommissionPreprocessedController.$inject = ['$rootScope', '$scope', '$sce', '$state', 'common', 'config', '$filter', '$timeout', 'NavigationService', 'dialogs', 'TransactionApiService', 'CodeValueService', '$stateParams', '$q', 'VmsApiService', 'WorkflowApiService', 'commonDataService'];

    function VmsCommissionPreprocessedController($rootScope, $scope, $sce, $state, common, config, $filter, $timeout, NavigationService, dialogs, TransactionApiService, CodeValueService, $stateParams, $q, VmsApiService, WorkflowApiService, commonDataService) {

        $scope.$on('vmsPreprocessedDocumentChange', function (event, data) {
            if (data.entityTypeId == ApplicationConstants.EntityType.VmsCommissionImportedRecord) {
        ...
        function buildPromise(tableState, isTruncated) {
            if ($scope.documentPublicId) {
                ...
                return VmsApiService.getVmsCommissionImportedRecordsTable($scope.documentPublicId, oDataParams, tableState);
            }
            else {
                return $q.reject();
            }
        ...
        $scope.markAsProcess = function (entity) {
            $scope.processing = true;
            VmsApiService.vmsCommissionImportRecordTypeUpdate({ LastModifiedDatetime: entity.LastModifiedDatetime, Id: entity.Id, VmsCommissionImportedRecordTypeIdFinal: ApplicationConstants.VmsCommissionImportedRecordType.ToProcess }).then(function (response) {
                var record = $scope.items[$scope.items.map(function (x) { return x.Id; }).indexOf(entity.Id)];
                if (record) {
                    record.VmsCommissionImportedRecordTypeIdFinal = ApplicationConstants.VmsCommissionImportedRecordType.ToProcess;
        ...
        $scope.markAsConflict = function (entity) {
            var dlg = dialogs.create('/Phoenix/modules/vms/views/VmsConflictReason.html', 'VmsConflictReasonController',
                {
                    recordType: ApplicationConstants.VmsCommissionImportedRecordType.Conflict,
                }, {});

            dlg.result.then(function (reason) {
                $scope.processing = true;
                VmsApiService.vmsCommissionImportRecordTypeUpdate(
                    {
                        Id: entity.Id,
                        LastModifiedDatetime: entity.LastModifiedDatetime,
                        VmsCommissionImportedRecordTypeIdFinal: ApplicationConstants.VmsCommissionImportedRecordType.Conflict,
                        UserNotes: reason
                    }
                )
                    .then(function (response) {
                        var record = $scope.items[$scope.items.map(function (x) { return x.Id; }).indexOf(entity.Id)];
                        if (record) {
                            record.VmsCommissionImportedRecordTypeIdFinal = ApplicationConstants.VmsCommissionImportedRecordType.Conflict;
        ...
        $scope.markAsDiscarded = function (entity) {
            var dlg = dialogs.create('/Phoenix/modules/vms/views/VmsConflictReason.html', 'VmsConflictReasonController', {
                recordType: ApplicationConstants.VmsCommissionImportedRecordType.Discarded,
            }, {});
                ...
                VmsApiService.vmsCommissionImportRecordTypeUpdate(
                    {
                        Id: entity.Id,
                        LastModifiedDatetime: entity.LastModifiedDatetime,
                        VmsCommissionImportedRecordTypeIdFinal: ApplicationConstants.VmsCommissionImportedRecordType.Discarded,
                        UserNotes: reason
                    }
                )
                    .then(function (response) {
                        var record = $scope.items[$scope.items.map(function (x) { return x.Id; }).indexOf(entity.Id)];
                        if (record) {
                            record.VmsCommissionImportedRecordTypeIdFinal = ApplicationConstants.VmsCommissionImportedRecordType.Discarded;
                            record.Reason = reason;
                            updateRecords(record);
                        }
                    }, function (error) { }).finally(function () {
        ...
        var updateRecords = function (record) {
            ...
            if ($scope.tableState.search && $scope.tableState.search.predicateObject) {
                if (!remove && $scope.tableState.search.predicateObject.VmsCommissionImportedRecordTypeIdFinal) {
                    if ($scope.tableState.search.predicateObject.VmsCommissionImportedRecordTypeIdFinal.indexOf(record.VmsCommissionImportedRecordTypeIdFinal) == -1) {
                        remove = true;
                    }
                }
            ...
            if (remove) {
                ...
                VmsApiService.getVmsCommissionImportedRecord(record.Id).then(
                    function (response) {
