(function (angular, app) {
    ...
    var controllerId = 'VmsUnitedStatesSourceDeductionProcessController';
    var vmsExceptionDialogOpen = false;
    var vmsItems = [];

    angular.module('phoenix.vms.controllers').controller('VmsUnitedStatesSourceDeductionProcessController', VmsUnitedStatesSourceDeductionProcessController);

    VmsUnitedStatesSourceDeductionProcessController.$inject = ['clientOrganizations', 'CodeValueService', 'common', 'dialogs', 'mixinsFactory', 'NavigationService', 'VmsApiService', '$stateParams', 'vmsTableParams', 'vmsNewTableState', 'WorkflowApiService', 'phoenixsocket', '$scope'];

    function VmsUnitedStatesSourceDeductionProcessController(clientOrganizations, CodeValueService, common, dialogs, mixinsFactory, NavigationService, VmsApiService, $stateParams, vmsTableParams, vmsNewTableState, WorkflowApiService, phoenixsocket, $scope) {

        NavigationService.setTitle('VMS United States Source Deduction Processing', 'icon icon-transaction');
        ...
        mixinsFactory.createTableSupportMixin(angular.extend({
            serviceMethod: VmsApiService.getUnitedStatesSourceDeductionProcessedRecords
        ...
        self.workflowBatchOperationOnTasks = {
            notifyName: {
                NotifyName_BatchOperation_OnBatchMarkered: 'VmsUnitedStatesSourceDeductionTransactionCreateBatchMark',
        ...
        function createTransaction() {
            ...
            var taskIds = _.chain(self.vmsBatchArray)
                ...
                .filter(['TaskResultId', ApplicationConstants.TaskResult.VmsUnitedStatesSourceDeductionRecordGenerateTransaction])
            ...
            WorkflowApiService.workflowBatchOperationOnTasksSelected({
                ...
                CommandBatchPreExecutionJsonBody: {
                    CommandName: 'BatchPreExecutionOnVmsUnitedStatesSourceDeductionProcessedRecordToPaymentTransaction',
                ...
                CommandBatchThreadExecutionJsonBody: {
                    CommandName: 'BatchThreadExecutionOnVmsUnitedStatesSourceDeductionRecordToPaymentTransaction'
                },
            }).then(
                function (success) {
        ...
        function sendToConflict(item, event) {
            ...
            dlg.result.then(function (reason) {
                self.viewLoading = true;
                VmsApiService.VmsUnitedStatesSourceDeductionRecordSetConflictType({ WorkflowPendingTaskId: item.WorkflowPendingTaskId, VmsUnitedStatesSourceDeductionRecordId: item.Id, ConflictReason: reason }).then(
                    function (success) {
                        self.vmsBatchArray = [];
        ...
        function retrieveClients() {
            self.viewLoading = true;
            var vmsDataParams = oreq.request().withSelect(['OrganizationIdInternal', 'OrganizationIdClient', 'ClientOrgDisplayName', 'UnitedStatesSourceDeductionPreparedCount']).url();
            VmsApiService.getVmsAllItems(vmsDataParams).then(
                function (response) {
                    var items = _.filter(response.Items, function (item) {
                        return item.UnitedStatesSourceDeductionPreparedCount > 0 && item.OrganizationIdInternal == $stateParams.internalOrganizationId;
                    });
        ...
        phoenixsocket.onPrivate(self.workflowBatchOperationOnTasks.notifyName.NotifyName_BatchPreExecution_OnReleased, function (event, data) {
            if (vmsExceptionDialogOpen === false && data.ExceptionEntityIds.length > 0) {
                ...
                dialogs.create('/Phoenix/modules/vmsUnitedStatesSourceDeduction/views/VmsUnitedStatesSourceDeductionProcessExceptionDialog.html', 'VmsUnitedStatesSourceDeductionProcessExceptionDialogController', { validationEventData: data, records: vmsItems }).result.then(
                    function (result) {
                        if (result.action == "ok") {
...
if (unregister) { unregisterFunctionList.push(unregister); } });
    ...
    app.resolve.VmsUnitedStatesSourceDeductionProcessController = {
    clientOrganizations: ['$q', 'VmsApiService', '$stateParams', function ($q, VmsApiService, $stateParams) {
        var result = $q.defer();
        var vmsDataParams = oreq.request()
            .withSelect(['OrganizationIdInternal', 'OrganizationIdClient', 'ClientOrgDisplayName', 'UnitedStatesSourceDeductionPreparedCount'])
            .url();
        VmsApiService.getVmsAllItems(vmsDataParams).then(
                function (response) {
                    var items = _.filter(response.Items, function (item) {
