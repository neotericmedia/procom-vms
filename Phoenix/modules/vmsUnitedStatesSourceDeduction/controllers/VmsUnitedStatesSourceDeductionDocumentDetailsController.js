(function () {
    ...
    angular.module('phoenix.vms.controllers').controller('VmsUnitedStatesSourceDeductionDocumentDetailsController', VmsUnitedStatesSourceDeductionDocumentDetailsController);

    /** @ngInject */
    VmsUnitedStatesSourceDeductionDocumentDetailsController.$inject = ['$state', 'CodeValueService', 'VmsApiService', 'vmsTableParams', 'mixinsFactory', 'NavigationService', 'dialogs', 'WorkflowApiService', 'phoenixsocket', 'common', 'vmsNewTableState', '$scope'];

    function VmsUnitedStatesSourceDeductionDocumentDetailsController($state, CodeValueService, VmsApiService, vmsTableParams, mixinsFactory, NavigationService, dialogs, WorkflowApiService, phoenixsocket, common, vmsNewTableState, $scope) {
        ...
        phoenixsocket.onPrivate("VmsUnitedStatesSourceDeductionManagementConflictsBatchMark", function (event, data) {
        ...
        phoenixsocket.onPrivate("VmsUnitedStatesSourceDeductionManagementConflictsProcessCompleted", function (event, data) {
        ...
        mixinsFactory.createTableSupportMixin(angular.extend({
            serviceMethod: VmsApiService.getUnitedStatesSourceDeductionProcessedRecordsByDocument
        ...
        self.successfulRetrieval = function (items) {
            ...
            _.filter(items, function (item) {
                return item.VmsUnitedStatesSourceDeductionImportedRecordTypeId == ApplicationConstants.VmsUnitedStatesSourceDeductionImportedRecordType.Discarded;
                });

            self.conflictRecords =
            _.filter(items, function (item) {
                return item.VmsUnitedStatesSourceDeductionImportedRecordTypeId == ApplicationConstants.VmsUnitedStatesSourceDeductionImportedRecordType.Conflict;
            });

            if (self.notifyParent) {
                self.notifyParent = false;
                $scope.$emit('onVmsUnitedStatesSourceDeductionManagementSkippedRecordsUpdate', self.recordsToProcess.length - self.skippedRecordsAfterProcess.length);

                $state.reload();
            }
        };

        self.loadSkippedPromise = VmsApiService.getVmsUnitedStatesSourceDeductionSummaryDocument(self.documentId).then(
            function (response) {
                if (response.VmsUnitedStatesSourceDeductionImportedRecords) {
                    self.skippedRecords = _.filter(response.VmsUnitedStatesSourceDeductionImportedRecords, function (item) {
                        return item.VmsUnitedStatesSourceDeductionImportedRecordTypeId == ApplicationConstants.VmsUnitedStatesSourceDeductionImportedRecordType.Discarded;
                    });
                }
