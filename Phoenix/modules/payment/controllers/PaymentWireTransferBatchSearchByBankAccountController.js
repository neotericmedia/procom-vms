...
(function (app, angular) {
    ...
    var controllerId = 'PaymentWireTransferBatchSearchByBankAccountController';
    ...
    angular.module('phoenix.payment.controllers').controller(controllerId,
        ['$scope', '$state', '$stateParams', 'NavigationService', 'PaymentApiService', 'CodeValueService',
            PaymentWireTransferBatchSearchByBankAccountController]);

    function PaymentWireTransferBatchSearchByBankAccountController($scope, $state, $stateParams, NavigationService, PaymentApiService, CodeValueService) {
        ...
        $scope.callServer = function (tableState) {
            ...
            var oDataParams = oreq.request()
                ...
                .url(); 
            var promise = PaymentApiService.getPaymentWireTransferBatchesByBankAccountAndBatchStatus($stateParams.bankAccountId, /*$stateParams.batchStatusId,*/ tableState, oDataParams)
               .then(function (response) {
                   NavigationService.setTitle(
                       (response.Items.length > 0 ? response.Items[0].InternalOrganizationBankAccountBankName + ' - ' : '') +
                       'Wire Transfer Batches', 'icon icon-payment');
        ...
        $scope.onClick = function (item) {
            $state.go('payment.wiretransferbatch.management', { batchId: item.Id });
        };
