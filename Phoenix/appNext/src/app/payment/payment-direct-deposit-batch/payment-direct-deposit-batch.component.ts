...
@Component({
  selector: 'app-payment-direct-deposit-batch',
  templateUrl: './payment-direct-deposit-batch.component.html',
  styleUrls: ['./payment-direct-deposit-batch.component.less']
})
export class PaymentDirectDepositBatchComponent extends PaymentBatchBaseComponent implements OnInit {
  protected currencyColumnFormat = { type: 'fixedPoint', precision: 2 };
  batchId: number;
  _dataGridComponentNamePayment: string = 'PaymentDirectDepositBatchPayment';
  _dataGridComponentNameTransaction: string = 'PaymentDirectDepositBatchTransaction';
  ...
  ngOnInit() {
    this.workflowGenerateFileButtonText = 'Generate Direct Deposit File';
  ...
  checkPaymentBatchExist(control: AbstractControl) {
    return this.paymentService
      .getPaymentDirectDepositDraftBatchesByBankAccount(control.value)
      .then((res: any) => {
  ...
  get getComponentName() {
    return 'PaymentDirectDepositBatchComponent';
  }
  protected getPaymentBatch(onSuccess: (data: any) => void) {
    ...
    const result = this.paymentService.getPaymentDirectDepositBatch(this.batchId, oDataParams).subscribe((data: any) => {
      this.navigationService.setTitle('payments-managedd', [data.InternalOrganizationLegalName, this.currencyCode, this.batchId]);
  ...
  onPaymentDataReceived(data: any) {
    // fix me
    // this.gridPayment.repaint();
  }
  ...
  protected discardPaymentBatch(onSuccess: (data: any) => void) {
    this.paymentService
      ...
      .then(response => {
        ...
        this.router.navigate(['/next', 'payment', 'directdepositbatch', 'searchbybankaccount', this.model.InternalOrganizationBankAccountId, this.currencyId]);
      })
      .catch(error => {
        console.log(error);
      });
  ...
  onSuccess_Batch_Finalize(data) {
    ...
    this.router.navigate(['/next', 'payment', 'directdepositbatch', 'searchbybankaccount', this.batchDetailsModalFormData.bankId, this.currencyId]);
    // this.$state.go('payment.directdepositbatch.management', { batchId: this.batchId });
  }
  ...
  onActionClick(stateActionId) {
    ...
    switch (currentStateAction.code) {
      ...
      case 'PaymentReleaseBatchGenerateFile':
            window.location.assign(this.paymentService.getPaymentDDStream(this.batchId));
          break;
      default:
        break;
    }
