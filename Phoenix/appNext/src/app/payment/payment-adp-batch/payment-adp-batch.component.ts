...
import { PhxDataTableConfiguration, PhxDataTableStateSavingMode, PhxDataTableColumn } from '../../common/model';
...
@Component({
  selector: 'app-payment-adp-batch',
  templateUrl: './payment-adp-batch.component.html',
  styleUrls: ['./payment-adp-batch.component.less']
})
export class PaymentADPBatchComponent extends PaymentBatchBaseComponent implements OnInit {
  protected currencyColumnFormat = { type: 'fixedPoint', precision: 2 };
  batchId: number;
  _dataGridComponentNamePayment: string = 'PaymentADPBatchPayment';
  _dataGridComponentNameTransaction: string = 'PaymentADPBatchTransaction';
  ...
  model: any = {};
  pageTitle: string;
  @ViewChild('changeBatchDetailsModal') changeBatchDetailsModal: PhxModalComponent;
  @ViewChild('navBar') navBar: PhxNavigationBarComponent;
  @ViewChild('gridPayment') gridPayment: PhxDataTableComponent;
  @ViewChild('gridTransaction') gridTransaction: PhxDataTableComponent;
  ...
  ngOnInit() {
    this.workflowGenerateFileButtonText = 'Generate ADP File';
  ...
  get getComponentName() {
    return 'PaymentADPBatchComponent';
  }
  protected getPaymentBatch(onSuccess: (data: any) => void) {
    const oDataParams = this.oreq
      .request()
      .withSelect(['Id', 'BatchNumber', 'DepositDate', 'CreateDate', 'Amount', 'CurrencyId', 'BatchStatusId', 'GarnisheeBankAccountId', 'OrganizationIdInternal', 'InternalOrganizationLegalName', 'GarnisheeUniqueCount', 'GarnisheeAmount'])
      .url();

    const result = this.paymentService.getPaymentAdpBatch(this.batchId, oDataParams).subscribe((data: any) => {
  ...
  protected discardPaymentBatch(onSuccess: (data: any) => void) {
    this.paymentService
      ...
      .then(response => {
        ...
        this.router.navigate(['/next', 'payment', 'adp', this.model.OrganizationIdInternal, this.currencyId]);
      })
      .catch(error => {
        console.log(error);
      });
  ...
  protected finalizePaymentBatch(onSuccess: (data: any) => void, onError: (err: any) => void): void {
    ...
    this.paymentService
      .finalizeDraftBatchPayment(this.batchId, this.batchDetailsModalFormData.bankId, this.notifyName.NotifyName_BatchFinalize_OnError, this.notifyName.NotifyName_BatchFinalize_OnSuccess, this.notifyName.NotifyName_BatchFinalize_OnProgress)
      .then(response => {
  ...
  recallPayment(item: any) {
    this.recallPaymentWithConfirmation(item)
      .then(r => {
        this.loadPaymentBatch();
        this.gridPayment.refresh();
      })
