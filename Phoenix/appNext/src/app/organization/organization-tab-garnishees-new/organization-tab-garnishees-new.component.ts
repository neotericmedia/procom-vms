...
@Component({
  selector: 'app-organization-tab-garnishees-new',
  templateUrl: './organization-tab-garnishees-new.component.html',
  styleUrls: ['./organization-tab-garnishees-new.component.less']
})

export class OrganizationTabGarnisheesNewComponent extends BaseComponentOnDestroy implements OnInit {
  ...
  getListGarnisheePayToGroup() {
    this.organizationService.getListGarnisheePayToGroup()
      .pipe(takeUntil(this.isDestroyed$))
      .subscribe((response: any) => {
  ...
  formGroupInitialValues() {
    this.garnishee = {
      ...
      GarnisheeOrganizationIdSupplier: this.organizationId,
      PayPercentage: null,
  ...
  onClickSubmit() {
    ...
    this.organizationService.garnisheeNewSave(this.newGarnishee)
      .pipe(takeUntil(this.isDestroyed$))
      .subscribe((response: any) => {
        this.canShow = false;
        ...
        this.outputEvent.emit(this.garnishee);
        this.isNew = false;
        this.isPayeeEditable = true;
        this.modal.hide();
      }, error => {
    ...
    const formGroup = formGroupSetup.hashModel.getFormGroup<IGarnisheeNew>(formGroupSetup.toUseHashCode, 'IGarnisheeNew', garnishee, 0, () =>
      formGroupSetup.formBuilder.group<IGarnisheeNew>({
        ...
        GarnisheeOrganizationIdSupplier: [
          garnishee.GarnisheeOrganizationIdSupplier,
          [
            ValidationExtensions.required(formGroupSetup.customFieldService.formatErrorMessage('GarnisheeOrganizationIdSupplier', CustomFieldErrorType.required))
          ]
        ],
