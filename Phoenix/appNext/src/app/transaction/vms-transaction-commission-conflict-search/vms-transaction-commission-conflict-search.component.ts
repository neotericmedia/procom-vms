...
import { CommonService, PhxLocalizationService , DialogService } from '../../common/index';
...
@Component({
    selector: 'app-vms-transaction-commission-conflict-search',
    templateUrl: './vms-transaction-commission-conflict-search.component.html',
    styleUrls: ['./vms-transaction-commission-conflict-search.component.less']
})
export class VmsTransactionCommissionConflictSearchComponent implements OnInit, OnDestroy {
    ...
    actionsArr: any = [
        { Id: 1, actionId: PhxConstants.StateAction.VmsCommissionProcessedRecordIntercompanyTransfer, DisplayName: 'Inter-Company Change' },
        { Id: 2, actionId: PhxConstants.StateAction.VmsCommissionProcessedRecordDiscard, DisplayName: 'Delete Record' },
        { Id: 3, actionId: PhxConstants.StateAction.VmsCommissionProcessedRecordAutoResolve, DisplayName: 'Revalidate Records' }
    ];
    ...
    dataTableConfigurationWorkOrderMappings: PhxDataTableConfiguration = new PhxDataTableConfiguration({
        ...
        noDataText: 'There are no Work Orders associated with selected Vms Commission item.'
    });
    oDataParams: string;
    ...
    ngOnInit() {
        ...
        this.navigationService.setTitle('thirdpartyimport-commission-conflict');
    ...
    getOrganizationClients() {
        const orgClientDataParams = oreq.request().withSelect(['CommissionConflictCount', 'ClientOrgDisplayName', 'OrganizationIdClient', 'OrganizationIdInternal'])
            .withFilter(oreq.filter('OrganizationIdInternal').eq(this.organizationIdInternal).and().filter('CommissionConflictCount').gt(0)).url();
    ...
    newClientOrgSelected(id) {
        if (id) {
            this.router.navigate(['/next', 'transaction', 'vms-commission-conflict', 'search', this.organizationIdInternal, 'client', id]);
    ...
    organizationIdClientRemoved() {
        this.router.navigate(['/next', 'transaction', 'vms-commission-conflict', 'search', this.organizationIdInternal]);
    ...
    actionSelect(actionId) {
        ...
        const isRevalidate = actionId === PhxConstants.StateAction.VmsCommissionProcessedRecordAutoResolve;
    ...
    dxRowClick(e) {
        if (this.dataTableConfiguration.selectionMode === PhxDataTableSelectionMode.Single) {
            ...
            this.WorkOrderMappingDataset = _.cloneDeep(rowData.VmsCommissionWorkOrders);
            this.WorkOrderMappingDatasetOriginal = rowData.VmsCommissionWorkOrders;
    ...
    dataReceived(data: any[]) {
        ...
        //    this.actions.push({ Id: 3, CommandName: 'VmsCommissionRevalidateRecords', DisplayName: 'Revalidate Records' });
        // }
    }
    ...
    validateAndResolve() {
        ...
        if (conflictedRows.length > 0) {
            ...
            if (errors.length > 0) {
                ...
                const commandName = this.codeValueService.getCodeValueCode(PhxConstants.StateAction.VmsCommissionProcessedRecordManualResolve, this.commonService.CodeValueGroups.StateAction);
    ...
    refreshAndDecreaseCount(decreaseCount: number) {
        ...
        if (org) {
            org.CommissionConflictCount = Math.max(org.CommissionConflictCount - decreaseCount, 0);
    ...
    onContentReady(event) {
        const disabled: boolean = this.WorkOrderMappingDatasetParent.ValidationMessages.indexOf('not Commission Eligible') >= 0;
            ...
            Buttons: [
                { Id: 1, Name: 'Cancel', SortOrder: 1, Class: 'btn-default' },
                {
                    ...
                    ClickEvent: (callBackObj: PhxDialogComponentEventEmitterInterface) => {
                        this.vmsService.updateVmsCommissionProcessedRecordUserNotes({
    ...
    initStateActions() {
        ...
        this.stateActions = [
          { // change Internal Org
            actionId: PhxConstants.StateAction.VmsCommissionProcessedRecordIntercompanyTransfer,
          ...
          { // Discard
            actionId: PhxConstants.StateAction.VmsCommissionProcessedRecordDiscard,
          ...
          { // Resolve
            actionId: PhxConstants.StateAction.VmsCommissionProcessedRecordManualResolve,
            displayText: 'Resolve',
            style: StateActionButtonStyle.PRIMARY,
            skipSecurityCheck: true,
            hiddenFn: (action, componentOption) => {
          ...
          { // cancel
            ...
            hiddenFn: (action, componentOption) => {
              return !(
                [PhxConstants.StateAction.VmsCommissionProcessedRecordIntercompanyTransfer
                  , PhxConstants.StateAction.VmsCommissionProcessedRecordDiscard
                  , PhxConstants.StateAction.VmsCommissionProcessedRecordManualResolve].includes(self.selectedAction)
