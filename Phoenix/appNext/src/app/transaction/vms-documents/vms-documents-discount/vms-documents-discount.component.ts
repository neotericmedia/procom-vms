...
@Component({
  selector: 'app-vms-documents-discount',
  templateUrl: './vms-documents-discount.component.html'
})
export class VmsDocumentsDiscountComponent extends BaseComponentActionContainer implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.actionId = PhxConstants.StateAction.VmsDiscountProcessedRecordDiscard;
  }
  ...
  getConflictIds(): number[] {
    return this.details.phxTable
      ...
      .items()
      .filter(item => {
        return item.VmsDiscountImportedRecordTypeId === PhxConstants.VmsImportedRecordType.Conflict;
      })
      .map(item => item.Id);
  }
  ...
  discardAllConflicts() {
    ...
    this.dialogService.confirm('Discard Records', dialogHeader).then(button => {
      if (button === DialogResultType.Yes) {
        // this.executeBatch({ CommandName: 'BatchPreExecutionOnVmsDiscountRecordToDiscardedType', WorkflowPendingTaskId: -1 }, { CommandName: 'BatchThreadExecutionOnVmsDiscountRecordToDiscardedType' });
  ...
  onRowClick(event: any) {
    ...
    if (recordId) {
      this.router.navigate(['/next', 'vms', 'batch', 'discount', 'processed', recordId, 0]);
    }
  }

  private listenForProcessCompleted() {
    this.signalrService
      .onPrivate('VmsDiscountManagementConflictsProcessCompleted', () => {
  ...
  private listenForBatchMark() {
    this.signalrService
      .onPrivate('VmsDiscountManagementConflictsBatchMark', () => {
  ...
  private initialize(reload: boolean) {
    this.activatedRoute.params.subscribe(params => {
      this.vmsDocumentService
        .getVmsDiscountSummaryDocument(params['documentId'])
        .takeWhile(() => true)
        .subscribe((document: any) => {
          this.document = document;
          this.skippedRecords = this.document.VmsDiscountImportedRecords.filter(item => {
            return item.VmsDiscountImportedRecordTypeId === PhxConstants.VmsImportedRecordType.Discarded;
          });
          this.dataSourceUrl = `vms/getDiscountProcessedRecordsByDocument/document/${this.document.DocumentId}`;
          if (reload) {
