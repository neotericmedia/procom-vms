...
@Component({
  selector: 'app-vms-documents-batch-commission',
  templateUrl: './vms-documents-batch-commission.component.html',
  styleUrls: ['./vms-documents-batch-commission.component.less']
})
export class VmsDocumentsBatchCommissionComponent extends BaseComponentOnDestroy implements OnInit {
  record: any;
  ...
  ngOnInit() {
    this.activatedRoute.params.takeUntil(this.isDestroyed$).subscribe(params => {
      this.vmsDocumentService
        .getVmsCommissionProcessedRecordById(params['recordId'])
  ...
  initDataSource(record) {
    ...
    this.phxTableWorkOrderDataSource = record && record.WorkOrder ? [record.WorkOrder] : null;
    this.phxTableTransactionDataSource = record && record.BillingTransaction ? [record.BillingTransaction] : null;
    this.transactionHeaderId = record.TransactionHeaderId;
  }
  ...
  onProcessedRecordClick(event) {
    if (event && event.data) {
      ...
      this.router.navigate(['/next', 'vms', 'commission', 'InternalOrganization', OrganizationIdInternal, 'document', DocumentId, 'details']);
    }
  }
  ...
  onTransactionClick(event) {
    if (event && event.data) {
      this.router.navigate(['/next', 'transaction', this.transactionHeaderId, 'summary']);
    }
  }
}
